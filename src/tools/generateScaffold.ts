...
async function generateAndroidProject(
  ...
  // Generate MainActivity.kt (always generated - unified architecture)
...
/**
 * Generate MainActivity.kt with unified architecture
 * Always extends WebViewActivity with getWebAppUrl() override
 * Supports automatic DEBUG/RELEASE URL switching
 */
function generateMainActivityKt(activityHandlers: HandlerInfo[]): string {
  // Generate imports (only if there are handlers)
  const imports = activityHandlers.length > 0
    ? activityHandlers.map(h => `import com.template.bridge.${h.className}`).join('\n') + '\n'
    : '';

  // Generate handler initialization
  const handlerInits = activityHandlers.length > 0
    ? activityHandlers.map(h => {
        const varName = h.className.charAt(0).toLowerCase() + h.className.slice(1);
    ...
    : '        // No activity-based handlers configured';
...
${imports}/**
 * Main Activity - Entry point for the hybrid web app
 * Auto-generated by HWMS based on selected modules
 *
 * This activity extends WebViewActivity and serves as the app entry point.
 * WebView is configured automatically in the parent class.
 *
 * URL Configuration:
 * - DEBUG build: Automatically uses development server (configurable via DEV_SERVER_URL)
 * - RELEASE build: Uses bundled assets/index.html
 */
class MainActivity : WebViewActivity() {

    companion object {
        /**
         * ⚠️ Development server URL - 웹 서버 포트에 맞게 수정 필요!
         *
         * 포트 번호를 웹 개발 서버 포트와 일치시키세요:
         * - Vite 기본값: 5173
         * - Create React App / Next.js: 3000
         * - 기타: package.json의 dev 스크립트 확인
         *
         * 에뮬레이터: 10.0.2.2는 호스트 PC의 localhost를 가리킴
         * 실제 기기: PC의 로컬 IP로 변경 (예: "http://192.168.0.100:3000")
         */
        private const val DEV_SERVER_URL = "http://10.0.2.2:5173"
    }

    /**
     * Configure the web app URL based on build type
     *
     * DEBUG: Uses DEV_SERVER_URL (development server)
     * RELEASE: Uses bundled assets/index.html
     *
     * To test with a real device in DEBUG mode:
     * 1. Change DEV_SERVER_URL to your computer's local IP
     * 2. Ensure your device and computer are on the same network
     * 3. Run 'npm run dev -- --host' in the web folder
     */
    override fun getWebAppUrl(): String {
